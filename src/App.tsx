...
const login = async () => {
  try {
    ...
    window.open(
      `https://www.themoviedb.org/authenticate/${tokenResponse.data.request_token}?redirect_to=${baseUrl}/login`,
      '_self'
    );
  } catch (error) {
    console.log(error);
...
function App() {
  ...
  const onLogin = useCallback((sessionIdProp: string) => {
    setSessionId(sessionIdProp);
  }, []);

  const logout = async () => {
    try {
      await tmdbApiClient.delete('authentication/session', {
        ...
        <div className="NavBar">
          <div className="Container">
            <div className="InnerNavBar">
              ...
              {sessionId && (
                <div>
                  <Link className="MyListsLink" to="/lists">
                    My lists
                  </Link>
