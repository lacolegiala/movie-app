...
const MovieFullInfo: React.FC<MovieInfoProps> = (props: MovieInfoProps) => {
    ...
    try {
      ...
      if (window.localStorage.getItem('movie_app/sessionId')) {
        ...
        const listResults = listResponse.data.results.map((list) => {
          return tmdbApiClient.get<{ item_present: boolean }>(
            `/list/${list.id}/item_status?movie_id=${movieInfo.data.id}`
          );
        });
        const itemStatuses = await Promise.all(listResults);
        const listsMovieIsNotAddedTo = listResponse.data.results.filter(
          (list, index) => !itemStatuses[index].data.item_present
        );
  ...
  const addToList = async (listId: number, movieId: number) => {
    try {
      await tmdbApiClient.post(`list/${listId}/add_item`, {
        media_id: movieId,
      });
