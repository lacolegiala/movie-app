...
const List: React.FC = () => {
  ...
  const getListDetails = useCallback(async () => {
    try {
  ...
  const deleteMovie = async (movieId: number, movieName: string) => {
    try {
      if (window.confirm(`Delete ${movieName}?`)) {
        await tmdbApiClient.post(`list/${id}/remove_item`, {
          media_id: movieId,
        });
        getListDetails();
      }
    ...
    <div className="Container">
      {listState.type === 'success' && (
        <div>
          ...
          <button className="SecondaryButton DeleteButton" onClick={deleteList}>
            Delete list
          </button>
                ...
                <div key={item.id}>
                  <Link className="PosterText" to={`/movies/${item.id}`}>
                    ...
                    <p className="SmallMargin">
                      {createReleaseYear(item.release_date)}
                    </p>
                  </Link>
                  <button onClick={() => deleteMovie(item.id, item.title)}>
                    Delete
                  </button>
                </div>
              ))
      ...
      {listState.type === 'loading' && <h1>Loading</h1>}
      {listState.type === 'error' && (
        <div>
          <h1>
            Something went wrong. Try again or check that the id is correct.
          </h1>
