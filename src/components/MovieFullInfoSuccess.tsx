...
const MovieFullInfoSuccess: React.FC<SuccessProps> = (props: SuccessProps) => {
  ...
  const youtubeTrailer = props.movieData.videos.results.find(
  ...
  return (
    <div className="PosterText">
      <div
        ...
        style={{
          backgroundImage: `linear-gradient(to bottom, rgba(227, 226, 226, 0), rgba(28, 27, 27, 0.90)), url(${createImageUrl(
      ...
      <div className="Container">
          ...
          <div>
            ...
            {props.lists.map((list) => (
              <button
                ...
                key={list.id}
                onClick={() => props.addToList(list.id, props.movieData.id)}
              >
                {list.name}
              </button>
            ))}
        ...
        {youtubeTrailer !== undefined && (
          <a
            className="PrimaryButton TrailerButton"
            href={`https://youtube.com/watch?v=${youtubeTrailer.key}`}
            rel="noreferrer"
            target="_blank"
          >
            Watch trailer
