...
const MovieListByGenre: React.FC = () => {
  ...
  const discoverMovies = useCallback(
    async () => {
      try {
        const genreResponse = await tmdbApiClient.get<{ genres: Genre[] }>(
          'genre/movie/list'
        );
        ...
        const moviesWithGenreResponse = await tmdbApiClient.get<{
          results: Movie[];
        ...
        setMovies({
          movies: movies.movies.concat(moviesWithGenreResponse.data.results),
          numberOfMovies: moviesWithGenreResponse.data.total_results,
        });
      } catch (error) {
        console.log(error);
      }
